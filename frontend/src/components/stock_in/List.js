...
const columns = [
  ...
  { field: 'amount', headerName: 'Quantidade', flex: 1, headerClassName: 'dg-header', cellClassName: 'stock-in-cell', type: 'number', valueGetter: (params) => `+ ${params.row.amount}` },
  { field: 'createdAt', headerName: 'Adicionado em', flex: 1, headerClassName: 'dg-header', type: 'dateTime', valueGetter: ({ value }) => value && new Date(value).toLocaleDateString('pt-BR') },
  {
    headerName: 'Ações',
    ...
    flex: 1,
    getActions: (params) => [
      <Link to={`/${process.env.REACT_APP_API_ROUTE_STOCK_IN}/detalhes/${params.id}`} >
        <GridActionsCellItem
...
export default function List(props) {
  ...
  useEffect(() => {
    ...
    async function getStockIn() {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/${process.env.REACT_APP_API_ROUTE_STOCK_IN}`, { headers });
      ...
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', minHeight: `${minHeight}vh` }}>
        <Grid container spacing={2} justifyContent="flex-end">
          <Grid item xs={6} style={{ display: 'flex', flexWrap: 'wrap', marginBottom: '20px' }}>
            <AddShoppingCartIcon />
            <h3 style={{ margin: 0, paddingLeft: '10px' }}>Entradas de estoque</h3>
          </Grid>
          <Grid item xs={6}>
            <Box display="flex" justifyContent="flex-end">
              <Link to={`/${process.env.REACT_APP_API_ROUTE_STOCK_IN}/novo`}>
                <Button variant="outlined">Nova entrada</Button>
              </Link>
            </Box>
          </Grid>
        </Grid>
