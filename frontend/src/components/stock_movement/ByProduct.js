...
export default function List(props) {
  ...
  useEffect(() => {
    ...
    async function getStockMovement() {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/${process.env.REACT_APP_API_ROUTE_STOCK_MOVE}/produto/${id}`, { headers });
      ...
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', minHeight: `${minHeight}vh` }}>
        <Grid container spacing={2} justifyContent="flex-end">
          <Grid item xs={12} style={{ display: 'flex', flexWrap: 'wrap', marginBottom: '20px' }}>
            <SyncAltIcon />
            <h3 style={{ margin: 0, paddingLeft: '10px' }}>Movimentação de estoque do produto "{productName}"</h3>
          </Grid>
        </Grid>
        <Box
          sx={{
